...
impl SpaceapiServer {
    ...
    fn route(self) -> Router {
        ...
        router.get("/", handlers::ReadHandler::new(self.status.clone(), self.redis_connection_info.clone(), self.sensor_specs.clone(), self.status_modifiers));
        router.put("/sensors/:sensor/", handlers::UpdateHandler::new(self.redis_connection_info.clone(), self.sensor_specs.clone()));
        router
    }
